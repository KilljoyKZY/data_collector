...
def job():
    ...
    for i in range(len(d_all)):
        city = d_all['city'].iloc[i]
        file_path = "/depot/cai161/data/Bike_Share_Data/Station_data/DataInFolders/"
        now=datetime.datetime.now()
        time_stamp = now.strftime("%Y-%m-%d_%H:%M:%S")
        download_date = now.strftime("%Y-%m-%d")
        save_to_folder = file_path +city+'_data/'+city+'_'+download_date+'/'
        path = save_to_folder + '*.csv'
        csvx_list = glob.glob(path)
        if len(csvx_list) < 200:
            print("Error city {} with count".format(city, len(csvx_list)))
            error_count += 1
    if error_count > 0:
        raise NameError('Low collection encountered')
